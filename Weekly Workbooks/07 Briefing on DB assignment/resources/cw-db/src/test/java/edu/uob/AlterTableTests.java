...
public class AlterTableTests {
    ...
    public void testAlter() {
        ...
        assertTrue(response.contains("[ERROR]"), "Cannot drop a non-existing column in an empty table");
        ...
        assertTrue(syntaxConstructor.stringContainsCaseInsensitively(response, "id"), "ID column should be added automatically");
        assertTrue(syntaxConstructor.stringContainsCaseInsensitively(response, "name"), "name column should have been added");
        assertTrue(syntaxConstructor.stringContainsCaseInsensitively(response, "mark"), "name column should have been added");
        assertTrue(syntaxConstructor.stringContainsCaseInsensitively(response, "pass"), "pass column should have been added");

        response = sendCommandToServer(syntaxConstructor.alterCommand(randomTableName, "DROP", "name"));
        assertTrue(response.contains("[OK]"), "A valid query was made, however an [OK] tag was not returned");
        response = sendCommandToServer(syntaxConstructor.selectCommand("*", randomTableName));
        assertFalse(syntaxConstructor.stringContainsCaseInsensitively(response, "name"), "name column should have been deleted");

        response = sendCommandToServer(syntaxConstructor.alterCommand(randomTableName, "DROP", "mark"));
        assertTrue(response.contains("[OK]"), "A valid query was made, however an [OK] tag was not returned");
        response = sendCommandToServer(syntaxConstructor.selectCommand("*", randomTableName));
        assertFalse(syntaxConstructor.stringContainsCaseInsensitively(response, "mark"), "mark column should have been deleted");

        response = sendCommandToServer(syntaxConstructor.alterCommand(randomTableName, "DROP", "pass"));
        assertTrue(response.contains("[OK]"), "A valid query was made, however an [OK] tag was not returned");
        response = sendCommandToServer(syntaxConstructor.selectCommand("*", randomTableName));
        assertFalse(syntaxConstructor.stringContainsCaseInsensitively(response, "pass"), "pass column should have been deleted");
