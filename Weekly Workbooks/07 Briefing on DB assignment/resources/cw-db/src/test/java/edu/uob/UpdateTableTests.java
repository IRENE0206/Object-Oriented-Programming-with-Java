...
public class UpdateTableTests {
    ...
    public void testUpdateRecords() {
        String response = sendCommandToServer(syntaxConstructor.updateCommand(randomTableName, "mark = 50", "name == 'Bob'"));
        ...
        response = sendCommandToServer(syntaxConstructor.updateCommand(randomTableName, "mark = 50, pass= TRUE", "name == 'Bob'"));
        ...
        response = sendCommandToServer(syntaxConstructor.updateCommand(randomTableName, "tutor = Steve", "name == 'Bob'"));
        assertTrue(response.contains("[ERROR]"), "Invalid NameValuePair");
        response = sendCommandToServer(syntaxConstructor.updateCommand(randomTableName, "pass =False", "name == 'Anas'"));
        assertTrue(response.contains("[OK]"), "A valid query was made, however an [OK] tag was not returned");
        response = sendCommandToServer(syntaxConstructor.updateCommand(randomTableName, "pass =False", "tutor == 'Anas'"));
        assertTrue(response.contains("[ERROR]"), "Invalid condition");

        response = sendCommandToServer(syntaxConstructor.updateCommand(randomTableName, "id=3, mark=40", "name LIKE 'ive'"));
